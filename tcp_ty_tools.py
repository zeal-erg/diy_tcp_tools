...
import json
import binascii
# from get_mjpeg import PictDisplay
...
class Ui_TCP(QDialog):
    ...
    def __init__(self, st, pict, num):
        ...
        self.comboBox_tcp.addItem("")
        self.comboBox_tcp.addItem("")
        self.comboBox_tcp.addItem("")  # 添加TCP-TY客户端
    ...
    def layout_ui(self):
        ...
        self.h_box_5.addWidget(self.pushButton_else)
    ...
    def ui_translate(self, ):
        ...
        self.comboBox_tcp.setItemText(5, self._translate("TCP-UDP", "TCP-TY客户端"))   # 增加TCP-TY 客户端
        ...
        self.label_dir.setText(self._translate("TCP-UDP", "请选择index.html/保存 的文件夹"))
        self.label_written.setText(self._translate("TCP-UDP", "Designed By zerl.erg"))
    ...
    def combobox_change(self):
        ...
        if self.comboBox_tcp.currentIndex() == 5:
            self.label_sendto.show()
            self.lineEdit_ip_send.show()
            self.textEdit_send.show()
            self.label_dir.show()
    ...
    def rce_data(self, path, address):
        # 协议解析
        # 协议解析-TY-TCP
        # output = binascii.hexlify(msg)
        # print(output)
        # r_msg = msg.decode('utf-8')
        # # 接收图片数据
        # self.msg = '来自IP:{}端口:{}:\n{}\n'.format(address[0], address[1], r_msg)
        # self.signal_write_msg.emit("write")
        # 默认路径设置
        self.pict_man.pict_save(path)
    ...
    def tcp_ty_parse(self, msg):
        data = json.dumps(msg)
        # print(data)
        data_p = json.loads(msg)
        # print(data_p)
    ...
    def tcp_ty_client_concurrency(self):
        ...
        while True:
            ...
            if recv_msg:
                ...
                # self.rce_data(file_last, self.address)
                # msg = recv_msg.decode('utf-8')
...
class PictDisplay:
    ...
    def pict_exit(self):
        self._async_raise(self.save_display_th.ident, SystemExit)
